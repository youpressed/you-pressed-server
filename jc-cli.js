...
async function run(func, args, container, requireRunning = true) {
  ...
  if(isRunning || !requireRunning) {
    ...
    console.log(chalk.yellow(`The ${container} container does not seem to be running. Try to serve and then try again.`), chalk.green('$ yarn jc serve'));
    console.log(chalk.yellow('Run', chalk.green('$ yarn jc --help'), 'for more info'));
    console.log(chalk.yellow('If the containers failed to shut down, use the kill command'), chalk.green('$ yarn jc kill'));
  }
}
...
program
  ...
  .description('Run stellar commands on the python container. *Note, flags are not allowed. For full use of the stellar command, open a shell using yarn jc bash')
  .action(options => {
...
program
  ...
  .action(async name => {
    name = name || `you-pressed-${moment().unix()}`
...
program
  ...
  .description('Restore you-pressed db to specified snapshot')
  .action(async name => {
    if(!name) {
    ...
    console.log(chalk.yellow('Restoring you_pressed db to:'), chalk.red(name));
...
program
  ...
  .description('Run rails commands on the web container. *Note, flags are not allowed. For full use of the rails command, open a shell using yarn jc bash')
  .action(options => {
...
program
  ...
  .description('Run rake commands on the web container. *Note, flags are not allowed. For full use of the rails command, open a shell using yarn jc bash')
  .action(options => {
...
program
  .on('--help', function () {
    ...
    console.log('');
    console.log('    $ yarn jc serve');
    console.log('    $ yarn jc bash');
    console.log('    $ yarn jc bash --help');
    console.log('    $ yarn jc psql');
    console.log('    $ yarn jc console');
    console.log('    $ yarn jc c');
    console.log('    $ yarn jc reset');
    console.log('    $ yarn jc snapshot MY_SNAPSHOT');
    console.log('    $ yarn jc restore MY_SNAPSHOT');
    console.log('    $ yarn jc kill');
    console.log('    $ yarn jc rails g model Post title description date:datetime');
    console.log('    $ yarn jc rake test');
    console.log('    $ yarn jc rake migrate');
    console.log('    $ yarn jc rake db:rollback');
    console.log('    $ yarn jc wipe');
    console.log('    $ yarn jc tail');
    console.log('');
  });
